...
procedure bajas(var a:arch; n:integer);
var
	e:empleado;
	pos : integer;
	encontre : boolean;
begin
	...
	while(not eof(a) and (not encontre))do begin
		read(a,e);
		if(e.numero = n) then begin
			pos := filepos(a)-1;
			if(pos <> filesize(a)-1)then begin	
				seek(a, filesize(a)-1);		//voy saltando en el archivo
				read(a, e);
				seek(a, pos);
				write(a, e);
			end;
			seek(a, filesize(a)-1);	//esto lo voy a tener que hacer siempre
			truncate(a);	//baja fisica
		end;
	end;		
end;
...
BEGIN
	...
	repeat 
		...
		case y of
			 ...
			 8: begin
					writeln('Ingrese el codigo del registro que desea eliminar: ');
					readln(cod);
					bajas(a, cod);
				end				  	
		else 
